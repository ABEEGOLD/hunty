...
function CreateGameContent() {
  ...
  const [activeTab, setActiveTab] = useState<
    "create" | "rewards" | "publish" | "leaderboard"
  >("create");
  const [hunts, setHunts] = useLocalStorage<HuntDraft[]>("draft-hunts", [
    EMPTY_HUNT_DRAFT,
  ]);
  ...
  const handleTabChange = (
    newTab: "create" | "rewards" | "publish" | "leaderboard",
  ) => {
  ...
  useEffect(() => {
    ...
    toast.success(
      `Loaded ${template.title}. You can edit every field before publishing.`,
    );
  ...
  const setCoverImageUploadState = (
    huntId: number,
  ...
  const huntItemSchema = z.object({
    ...
    description: z
      .string()
      .min(8, "Clue description must be at least 8 characters."),
  ...
  const formSchema = z
    .object({
      ...
      rewards: z
        ...
        .min(1, "At least one reward slot is required."),
    })
    .refine(
      (data) => {
    ...
    .refine(
      (data) => {
        ...
        if (!isNaN(s.getTime()) && !isNaN(e.getTime()))
          return s.getTime() > Date.now() - 60_000 && e.getTime() > s.getTime();
        return false;
        return false;
      },
      {
    ...
    .refine(
      (data) =>
        data.rewards.reduce((sum, reward) => sum + reward.amount, 0) > 0,
      {
        message:
          "Reward pool must be greater than 0 and based on reward buckets.",
        path: ["rewards"],
      },
    );
  ...
  const handlePublish = async (formValues: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      const end_time = Math.floor(
        new Date(formValues.endDate).getTime() / 1000,
      );
      const description = formValues.hunts
      ...
      const nextStatus =
        start_time > Math.floor(Date.now() / 1000) ? "scheduled" : "active";
      ...
      addStoredHunt({
        ...
        rewardType: formValues.rewardType,
        rewardPool,
        rewards: formValues.rewards.map(({ place, amount }) => ({
          place,
          amount,
        })),
        rewardEscrowTxHash,
                      ...
                      >
                        ...
                        {errors.hunts && (
                          <div className="text-red-500 text-sm">
                            {errors.hunts.message ||
                              "At least 3 clues are required."}
                          </div>
                        )}
                        {errors.rewards && (
                          <div className="text-red-500 text-sm">

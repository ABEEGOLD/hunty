 ...
 */
const getProgressiveHintCost = (baseCost: number | undefined, level: number): number => {
  const safeBaseCost = typeof baseCost === "number" && baseCost > 0 ? baseCost : 5;
...
const recordHintUsage = (record: HintUsageRecord): void => {
  ...
  try {
    const existing = window.localStorage.getItem(HINT_ANALYTICS_STORAGE_KEY);

    const records: HintUsageRecord[] = existing ? JSON.parse(existing) : [];

    records.push(record);

    window.localStorage.setItem(HINT_ANALYTICS_STORAGE_KEY, JSON.stringify(records));
  } catch (error) {
...
export function PlayGame({
   ...
   */
  const [usedHintLevels, setUsedHintLevels] = useState<Record<number, number>>({});

  /**
   * Controls whether the current clue's hint is visible.
   */
  const [visibleHintLevel, setVisibleHintLevel] = useState<Record<number, number>>({});
        ...
        const localClue =
          ...
          localClues.find(
            (item) =>
              ...
              resolveLocalizedText(item.questionTranslations, locale, item.question) ===
                localizedQuestion
          );
...
>>>>>>> upstream/main
  ...
  const handleUseHint = () => {
    ...
    const highestUsedLevel = usedHintLevels[clueId] ?? 0;

    const nextHintLevel = highestUsedLevel + 1;

    const hintCost = getProgressiveHintCost(clue.hintCost, nextHintLevel);

    if (score < hintCost) {
      toast.error(`You need ${hintCost} points to use this hint.`);
      return;
    }
     ...
     */
    setScore((currentScore) => Math.max(0, currentScore - hintCost));
    ...
    toast.success(`Hint ${nextHintLevel} unlocked for ${hintCost} points.`);
  };
      ...
      <div className="min-h-screen bg-gradient-to-tr from-blue-100 bg-purple-100 to-[#f9f9ff] flex items-center justify-center">
        <div className="text-center rounded-3xl bg-white px-8 py-10 shadow-lg">
          ...
          <div className="flex items-center justify-center gap-3">
            {huntId != null && <Button onClick={() => refetch()}>{t("retry")}</Button>}
  ...
  if (!hasHunts) {
    return (
      <div className="min-h-screen bg-gradient-to-tr from-blue-100 bg-purple-100 to-[#f9f9ff] flex items-center justify-center">
        <div className="text-center rounded-3xl bg-white px-8 py-10 shadow-lg">
          <p className="text-slate-700 text-lg mb-4">{t("noClues")}</p>
    ...
    return (
      <div className="min-h-screen bg-gradient-to-tr from-blue-100 bg-purple-100 to-[#f9f9ff] flex items-center justify-center p-4">
        <div className="w-full max-w-md space-y-6 text-center rounded-3xl bg-white dark:bg-slate-900 px-8 py-10 shadow-lg border border-slate-100 dark:border-white/5">
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white">{t("huntEnded")}</h2>
  ...
  const currentClueUsedLevel = activeHunt ? (usedHintLevels[activeHunt.id] ?? 0) : 0;

  const currentVisibleHintLevel = activeHunt ? (visibleHintLevel[activeHunt.id] ?? 0) : 0;

  const nextHintLevel = currentClueUsedLevel + 1;

  const nextHintCost = activeHunt ? getProgressiveHintCost(activeHunt.hintCost, nextHintLevel) : 0;

  const hasHint = Boolean(activeHunt?.hint) && !solvedClues.has(activeHunt?.id ?? -1);

  const hasMoreHints = currentClueUsedLevel < 3;
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between gap-4">
                        <div>
                          <h3 className="font-semibold text-slate-900">Need a hint?</h3>

                          <p className="text-sm text-slate-500 mt-1">
                            Hint level {Math.min(currentClueUsedLevel, 3)} of 3
                          </p>
                        </div>

                        {currentClueUsedLevel > 0 && (
                          <span className="text-xs font-medium rounded-full bg-indigo-100 px-3 py-1 text-indigo-700">
                            {currentClueUsedLevel} used
                          </span>
                        )}
                      </div>

                      {currentVisibleHintLevel > 0 && activeHunt.hint && (
                        <div className="mt-4 rounded-xl bg-white border border-indigo-100 p-4">
                          <p className="text-sm font-medium text-indigo-600 mb-1">
                            Hint {currentVisibleHintLevel}
                          </p>

                          <p className="text-slate-700">{activeHunt.hint}</p>
                        </div>
                      )}
                      ...
                      {hasMoreHints ? (
                        <div className="mt-4 flex items-center justify-between gap-4">
                          ...
                          <Button
                            ...
                            disabled={score < nextHintCost}
                            className="rounded-full bg-gradient-to-b from-[#3737A4] to-[#0C0C4F] text-white"
                          >
                            <Lightbulb className="mr-2 h-4 w-4" />

                            {currentClueUsedLevel > 0 ? `Get Hint ${nextHintLevel}` : "Get Hint"}
                          </Button>
                        </div>
                      ) : (
                      ...
                      {score < nextHintCost && hasMoreHints && (
                        <p className="mt-2 text-xs text-red-500">
                          You need {nextHintCost - score} more points to unlock this hint.
                        </p>
                      )}
                    </div>
                  </div>
